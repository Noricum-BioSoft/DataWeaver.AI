...
from ..models.connector import ConnectorType, ConnectorStatus, AuthenticationType
...
class TestConnectorSystem:
    ...
    def test_get_connectors(self, client, db_session):
    ...
    def test_test_connection(self, client, db_session):
        ...
        response = client.post(f"/api/connectors/{connector_id}/test", json=test_request)
        assert response.status_code == 200
        data = response.json()
        assert data["success"] == True
        assert "Successfully connected" in data["message"]
    
    def test_discover_data_sources(self, client, db_session):
        ...
        response = client.post(f"/api/connectors/{connector_id}/discover")
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
        assert len(data) > 0
    ...
    def test_create_data_source(self, client, db_session):
        ...
        response = client.post(f"/api/connectors/{connector_id}/data-sources", json=data_source_data)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == data_source_data["name"]
        assert data["connector_id"] == connector_id
    
    def test_extract_data(self, client, db_session):
        ...
        ds_response = client.post(f"/api/connectors/{connector_id}/data-sources", json=data_source_data)
        data_source_id = ds_response.json()["id"]
        
        # Extract data
        extract_config = {"extract_type": "full"}
        response = client.post(f"/api/connectors/data-sources/{data_source_id}/extract", json=extract_config)
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "completed"
        assert data["data_source_id"] == data_source_id
    
    def test_sync_connector(self, client, db_session):
        ...
        client.post(f"/api/connectors/{connector_id}/data-sources", json=data_source_data)
        
        # Sync connector
        response = client.post(f"/api/connectors/{connector_id}/sync")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] == True
...
class TestDemoScenarios:
    ...
    def test_customer_intelligence_scenario(self, client, db_session):
        """Test Customer Intelligence scenario setup"""
        response = client.post("/api/connectors/scenarios/customer_intelligence/setup")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_project_management_scenario(self, client, db_session):
        """Test Project Management scenario setup"""
        response = client.post("/api/connectors/scenarios/project_management/setup")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_financial_consolidation_scenario(self, client, db_session):
        """Test Financial Consolidation scenario setup"""
        response = client.post("/api/connectors/scenarios/financial_consolidation/setup")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_biotech_research_scenario(self, client, db_session):
        """Test Biotech Research scenario setup"""
        response = client.post("/api/connectors/scenarios/biotech_research/setup")
        assert response.status_code == 200
        data = response.json()
...
class TestConnectorFactory:
    ...
    def test_connector_factory_registration(self, db_session):
        ...
        from ..services.connector_factory import ConnectorFactory, ConnectorType
    ...
    def test_connector_creation(self, db_session):
        ...
        from ..models.connector import Connector, ConnectorType, AuthenticationType
...
class TestEndToEndWorkflows:
    ...
    def test_complete_customer_intelligence_workflow(self, client, db_session):
        """Test complete Customer Intelligence workflow"""
        # 1. Setup scenario
        setup_response = client.post("/api/connectors/scenarios/customer_intelligence/setup")
        assert setup_response.status_code == 200
        setup_data = setup_response.json()
        
        # 2. Get connectors
        connectors_response = client.get("/api/connectors/")
        ...
        for connector in connectors:
            ...
            test_response = client.post(f"/api/connectors/{connector['id']}/test", json=test_request)
            assert test_response.status_code == 200
            test_data = test_response.json()
            assert test_data["success"] == True
        
        # 4. Discover data sources
        for connector in connectors:
            discover_response = client.post(f"/api/connectors/{connector['id']}/discover")
            assert discover_response.status_code == 200
            discovered_sources = discover_response.json()
            assert len(discovered_sources) > 0
        
        # 5. Sync data
        for connector in connectors:
            sync_response = client.post(f"/api/connectors/{connector['id']}/sync")
            assert sync_response.status_code == 200
            sync_data = sync_response.json()
            assert sync_data["success"] == True
    
    def test_complete_biotech_research_workflow(self, client, db_session):
        """Test complete Biotech Research workflow"""
        # 1. Setup scenario
        setup_response = client.post("/api/connectors/scenarios/biotech_research/setup")
        assert setup_response.status_code == 200
        setup_data = setup_response.json()
        ...
        test_response = client.post(f"/api/connectors/{lims_connector['id']}/test", json=test_request)
        assert test_response.status_code == 200
        test_data = test_response.json()
        assert test_data["success"] == True
        
        # 4. Discover LIMS data sources
        discover_response = client.post(f"/api/connectors/{lims_connector['id']}/discover")
        assert discover_response.status_code == 200
        discovered_sources = discover_response.json()
        assert len(discovered_sources) > 0
        
        # 5. Sync LIMS data
        sync_response = client.post(f"/api/connectors/{lims_connector['id']}/sync")
        assert sync_response.status_code == 200
        sync_data = sync_response.json()
        assert sync_data["success"] == True
